  ...
  return <div className="dashboard-shell">
    ...
    <main className="dashboard-main">
      ...
      {selectedModule.collection && selectedModule.pro && !isPro
        ...
        : selectedModule.collection && !['products','invoices','tasks'].includes(active) && <section className="content-card">
...
function ProductsView({ bakeryId, items }: { bakeryId: string; items: WorkspaceItem[] }) {
  ...
  const filtered = items.filter(item => (category === 'All' || item.category === category) && `${item.name || ''} ${item.sku || ''} ${item.category || ''}`.toLowerCase().includes(query.toLowerCase()))
  ...
  return <section className="app-section products-view">
    ...
    <div className="catalogue-meta"><strong>{filtered.length} item{filtered.length===1?'':'s'} in master catalogue</strong><span>Active products and services</span></div>
...
function InvoicesView({ bakeryId, invoices, products }: { bakeryId:string; invoices:WorkspaceItem[]; products:WorkspaceItem[] }) {
  const [creating,setCreating]=useState(false); const [preview,setPreview]=useState<WorkspaceItem|null>(null)
  ...
  async function markPaid(item:WorkspaceItem){await updateDoc(doc(db,'bakeries',bakeryId,'invoices',item.id),{status:'paid',updatedAt:serverTimestamp()})}
  return <section className="app-section invoice-view"><div className="app-page-title"><div><span className="eyebrow">BatchBoss Pro</span><h2>Invoicing</h2></div><button className="circle-add" onClick={()=>setCreating(true)}><Plus/></button></div><div className="invoice-stats"><article><span>Total billed</span><strong>{money.format(total)}</strong></article><article><span>Pending</span><strong>{money.format(pending)}</strong></article><article><span>Collected</span><strong>{money.format(collected)}</strong></article></div><div className="filter-chips"><button className="active">All</button><button>Pending</button><button>Paid</button><button>Overdue</button></div>{invoices.length?<div className="invoice-list">{invoices.map(item=><article className="invoice-card" key={item.id}><div><span className="invoice-number">{String(item.invoiceNumber||item.name||'Invoice')}</span><h3>{String(item.clientName||item.description||'Client')}</h3><p>{String(item.summary||'Invoice line items')}</p></div><span className={`status-pill ${item.status==='paid'?'paid':''}`}>{String(item.status||'pending')}</span><strong className="invoice-total">{money.format(Number(item.total||0))}</strong><div className="invoice-actions"><button onClick={()=>setPreview(item)}><Eye/> View</button>{item.status!=='paid'&&<button onClick={()=>markPaid(item)}>✓ Mark paid</button>}<button onClick={()=>navigator.share?.({title:String(item.invoiceNumber||'Invoice'),text:`Invoice total: ${money.format(Number(item.total||0))}`})}><Share2/></button></div></article>)}</div>:<EmptyState title="No invoices yet" text="Create a professional invoice using saved products and services." action="Create invoice" onAction={()=>setCreating(true)}/>} {creating&&<InvoiceEditor bakeryId={bakeryId} products={products} invoiceCount={invoices.length} onClose={()=>setCreating(false)}/>} {preview&&<InvoicePreview invoice={preview} onClose={()=>setPreview(null)}/>}</section>
}
...
function InvoiceEditor({bakeryId,products,invoiceCount,onClose}:{bakeryId:string;products:WorkspaceItem[];invoiceCount:number;onClose:()=>void}){
  const [productId,setProductId]=useState(products[0]?.id||''); const [quantity,setQuantity]=useState(1); const [discount,setDiscount]=useState(0); const [client,setClient]=useState(''); const [phone,setPhone]=useState('');
  const product=products.find(p=>p.id===productId); const unitPrice=Number(product?.sellingPrice||product?.price||0); const cost=Number(product?.costPrice||0); const subtotal=unitPrice*quantity; const total=Math.max(0,subtotal-discount); const profit=(unitPrice-cost)*quantity-discount
  async function save(event:FormEvent){event.preventDefault();const invoiceNumber=`INV-${String(invoiceCount+1).padStart(4,'0')}`;await addDoc(collection(db,'bakeries',bakeryId,'invoices'),{name:invoiceNumber,invoiceNumber,clientName:client,phone,dueDate:(document.getElementById('invoice-due') as HTMLInputElement)?.value,items:product?[{productId,name:product.name,quantity,unit:String(product.unit||'Each'),unitPrice,lineTotal:subtotal}]:[],summary:product?`${quantity} × ${product.name}`:'',subtotal,discount,total,estimatedProfit:profit,status:'pending',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});onClose()}
  return <Modal title="Create Invoice" onClose={onClose}><form className="modal-form invoice-form" onSubmit={save}><p className="modal-subtitle">Automatic line items &amp; totals</p><span className="form-section-label">Client &amp; invoice details</span><label>Client name<input value={client} onChange={e=>setClient(e.target.value)} required/></label><div className="split-fields"><label>Phone / WhatsApp<input value={phone} onChange={e=>setPhone(e.target.value)}/></label><label>Due date<input id="invoice-due" type="date" required/></label></div><span className="form-section-label">Line items</span>{products.length?<><label>Product or service<select value={productId} onChange={e=>setProductId(e.target.value)}>{products.map(p=><option key={p.id} value={p.id}>{String(p.name)} — {money.format(Number(p.sellingPrice||p.price||0))}</option>)}</select></label><label>Quantity<input type="number" min="1" value={quantity} onChange={e=>setQuantity(Number(e.target.value))}/></label></>:<div className="warning-note">Add a Product or Service first, then it will be available here.</div>}<section className="invoice-summary"><div><span>Subtotal</span><strong>{money.format(subtotal)}</strong></div><div><label>Discount (R)</label><input type="number" min="0" step=".01" value={discount} onChange={e=>setDiscount(Number(e.target.value))}/></div><div className="summary-total"><span>Total due</span><strong>{money.format(total)}</strong></div><div className="estimated-profit"><span>Estimated profit</span><strong>{money.format(profit)}</strong></div></section><button className="primary-button" disabled={!product}>Generate invoice</button></form></Modal>
}

function InvoicePreview({invoice,onClose}:{invoice:WorkspaceItem;onClose:()=>void}){
  const items=Array.isArray(invoice.items)?invoice.items as Array<Record<string,unknown>>:[]
  return <Modal title="Tax Invoice" onClose={onClose}><div className="invoice-preview"><div className="preview-heading"><div><span>Invoice</span><strong>{String(invoice.invoiceNumber||invoice.name||'')}</strong></div><span className="status-pill">{String(invoice.status||'pending')}</span></div><div className="billed"><span>Billed to</span><strong>{String(invoice.clientName||'Client')}</strong><small>{String(invoice.phone||'')}</small></div>{items.map((item,index)=><div className="preview-line" key={index}><span>{String(item.quantity||1)} × {String(item.name||'Item')}</span><strong>{money.format(Number(item.lineTotal||0))}</strong></div>)}<div className="preview-total"><span>Total amount due</span><strong>{money.format(Number(invoice.total||0))}</strong></div><div className="preview-buttons"><button className="primary-button" onClick={()=>window.print()}><Download/> Print / Save PDF</button><button className="outline-button" onClick={()=>navigator.share?.({title:String(invoice.invoiceNumber||'Invoice'),text:`Amount due: ${money.format(Number(invoice.total||0))}`})}><Share2/> Share</button></div></div></Modal>
}

function TasksView({bakeryId,items}:{bakeryId:string;items:WorkspaceItem[]}){
  const today=new Date(); const [selected,setSelected]=useState(today.getDate()); const [adding,setAdding]=useState(false); const year=today.getFullYear(), month=today.getMonth(); const days=new Date(year,month+1,0).getDate(); const first=new Date(year,month,1).getDay(); const selectedDate=`${year}-${String(month+1).padStart(2,'0')}-${String(selected).padStart(2,'0')}`; const dayItems=items.filter(i=>i.dueDate===selectedDate)
  async function save(event:FormEvent<HTMLFormElement>){event.preventDefault();const data=new FormData(event.currentTarget);await addDoc(collection(db,'bakeries',bakeryId,'tasks'),{name:String(data.get('name')),description:String(data.get('description')||''),dueDate:String(data.get('dueDate')),status:'pending',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});setAdding(false)}
  return <section className="app-section tasks-view"><div className="app-page-title"><div><span className="eyebrow">Bakery planner</span><h2>Tasks &amp; Calendar</h2><p>{items.length} total tasks recorded</p></div><button className="primary-button compact" onClick={()=>setAdding(true)}><Plus/> Add task</button></div><div className="calendar-card"><h3>{today.toLocaleDateString('en-ZA',{month:'long',year:'numeric'})}</h3><div className="weekdays">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=><span key={d}>{d}</span>)}</div><div className="calendar-grid">{Array.from({length:first},(_,i)=><span key={`blank-${i}`}/>)}{Array.from({length:days},(_,i)=>i+1).map(day=><button className={selected===day?'selected':''} onClick={()=>setSelected(day)} key={day}>{day}</button>)}</div><div className="calendar-key"><strong>Viewing: {new Date(year,month,selected).toLocaleDateString('en-ZA',{weekday:'long',day:'numeric',month:'short',year:'numeric'})}</strong><span>● Pending &nbsp; <i>● Done</i></span></div></div><div className="task-filters"><button className="active">All</button><button>Pending</button><button>In progress</button><button>Completed</button></div>{dayItems.length?<div className="task-list">{dayItems.map(item=><article key={item.id}><CalendarDays/><div><strong>{String(item.name)}</strong><p>{String(item.description||'')}</p></div><span>{String(item.status||'pending')}</span></article>)}</div>:<EmptyState title={`No tasks scheduled for ${selected} ${today.toLocaleDateString('en-ZA',{month:'short'})}`} text="Schedule cake baking, pastry batches, decorating, ingredient restocking or client order deliveries." action="Add task for this day" onAction={()=>setAdding(true)}/>} {adding&&<Modal title="Add bakery task" onClose={()=>setAdding(false)}><form className="modal-form" onSubmit={save}><label>Task name<input name="name" required/></label><label>Details<textarea name="description" rows={3}/></label><label>Date<input name="dueDate" type="date" defaultValue={selectedDate} required/></label><button className="primary-button">Save task</button></form></Modal>}</section>
}
...
function SettingsView({ profile }: { profile: UserProfile }) {
  const [business,setBusiness]=useState<Record<string,unknown>>({}); const [saved,setSaved]=useState(false)
